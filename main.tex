...
\begin{itemize}
    \item \textbf{Phần đầu khối:} Chứa siêu dữ liệu (metadata) quản lý khối, bao gồm:
    \begin{itemize}
        ...
        \item \textbf{Nonce và bits:} Các tham số phục vụ cho cơ chế đồng thuận (ví dụ trong Proof of Work).
    \end{itemize}
...
\begin{itemize}
    \item \textbf{Bằng chứng công việc (Proof of Work - PoW):} Các nút phải cạnh tranh giải một bài toán toán học cực khó bằng sức mạnh phần cứng máy tính (tìm giá trị nonce thích hợp). Người đầu tiên giải được sẽ giành quyền đóng gói khối và nhận phần thưởng \cite{antonopoulos}. Bằng chứng công việc bảo mật rất cao nhưng tiêu tốn tài nguyên điện năng khổng lồ và tốc độ xử lý giao dịch rất chậm (Bitcoin chỉ khoảng 7 TPS).
    \item \textbf{Bằng chứng cổ phần (Proof of Stake - PoS):} Thay thế việc khai thác bằng phần cứng bằng việc đặt cọc (stake) tiền điện tử. Hệ thống dựa vào tỷ lệ cổ phần đặt cọc để chọn ngẫu nhiên nút có quyền tạo khối mới. Bằng chứng cổ phần tiết kiệm năng lượng hơn Proof of Work, tốc độ nhanh hơn, nhưng có nguy cơ dẫn đến việc tích lũy tài sản (người giàu càng giàu và nắm quyền kiểm soát mạng).
    \item \textbf{Bằng chứng ủy quyền (Proof of Authority - PoA):} Thay vì sử dụng năng lực tính toán hay lượng tiền đặt cọc, cơ chế này dựa trên danh tính và uy tín của các nút xác thực. Chỉ một nhóm các nút đã được kiểm duyệt, định danh và cấp phép từ trước mới có quyền đóng gói khối. Bằng chứng ủy quyền mang lại tốc độ xử lý giao dịch cực kỳ cao, gần như không tiêu tốn tài nguyên phần cứng và rất phù hợp cho các mạng blockchain riêng tư hoặc blockchain liên minh, tuy nhiên phải đánh đổi bằng việc mang tính tập trung cao hơn so với bằng chứng công việc hay bằng chứng cổ phần.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Tích hợp vai trò Node:} Thay vì phân rã hệ thống thành các nút độc lập (như nút sắp xếp, nút lưu trữ và CA để cấp phép), Khoai Chain tích hợp tất cả các chức năng đồng thuận, định tuyến giao dịch và lưu trữ vào một module nút duy nhất. Điều này giúp giảm thiểu độ trễ giao tiếp qua mạng lưới và tiết kiệm tối đa RAM/CPU.
    \item \textbf{Cơ chế lưu trữ nhúng:} Thay vì yêu cầu thiết lập các container cơ sở dữ liệu bên ngoài (như CouchDB), Khoai Chain sử dụng cơ sở dữ liệu dạng key-value được nhúng trực tiếp vào lõi hệ thống. Quá trình đồng bộ trạng thái sổ cái diễn ra nội bộ, loại bỏ hoàn toàn các cấu hình kết nối mạng phức tạp giữa lõi và cơ sở dữ liệu.
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{3.5cm}|p{5.5cm}|p{5.5cm}|}
        ...
        \textbf{Kiến trúc Node} & Phân rã phức tạp (Peer, Orderer, CA hoạt động trên các vùng nhớ riêng). & Tích hợp vai trò Node, với tất cả vai trò xử lý, đồng thuận và lưu trữ nằm trong một tiến trình duy nhất. \\ \hline
...
\begin{enumerate}[label=\textbf{\alph*)}]
    ...
    \begin{table}[H]
        ...
        \begin{tabularx}{\textwidth}{|p{3.5cm}|X|}
            ...
            \textbf{Tên trường hợp sử dụng} & Khởi tạo mạng lưới (khởi tạo khối genesis) \\ \hline
        ...
        \end{tabularx}
        \caption{Đặc tả chi tiết trường hợp sử dụng khởi tạo mạng lưới Khoai Chain}
    ...
    \begin{table}[H]
        ...
        \begin{tabularx}{\textwidth}{|p{3.5cm}|X|}
            ...
            \textbf{Tác nhân} & Quản trị viên mạng, nút cha (nút quản lý) \\ \hline
        ...
        \end{tabularx}
        \caption{Đặc tả chi tiết trường hợp sử dụng tạo nút mới Khoai Chain}
    ...
    \begin{table}[H]
        ...
        \end{tabularx}
        \caption{Đặc tả chi tiết trường hợp sử dụng tạo khối Khoai Chain}
    ...
    \begin{table}[H]
        ...
        \begin{tabularx}{\textwidth}{|p{3.5cm}|X|}
            ...
            4. Nút tiếp nhận trả về mã băm (TxHash) cho ứng dụng khách để theo dõi. \\ \hline
            \textbf{Luồng ngoại lệ} & 
...
\begin{enumerate}[label=\textbf{\alph*)}]
    ...
    \begin{figure}[H]
        ...
        \caption{Sơ đồ trình tự khởi tạo mạng lưới Khoai Chain}
        \label{}
    \end{figure}
    \newpage
    ...
    \begin{figure}[H]
        ...
        \caption{Sơ đồ trình tự tạo nút mới Khoai Chain}
        \label{}
    \end{figure}
    \newpage
    ...
    \begin{figure}[H]
        ...
        \caption{Sơ đồ trình tự tạo khối Khoai Chain}
        \label{}
    \end{figure}
    \newpage
    ...
    \begin{figure}[H]
        ...
        \caption{Sơ đồ trình tự gửi giao dịch Khoai Chain}
        \label{}
    \end{figure}
    \newpage
    ...
    \begin{figure}[H]
        ...
        \caption{Sơ đồ trình tự truy vấn sổ cái Khoai Chain}
        \label{}
    \end{figure}
\end{enumerate}
...
\begin{itemize}
    ...
    Mạng lưới ngang hàng phải hỗ trợ cơ chế tự động khám phá và kết nối giữa các nút thông qua giao thức truyền bá thông điệp. Khi có một nút mới gia nhập hoặc ngắt kết nối đột ngột, các nút còn lại phải tự động cập nhật topology mạng và đồng bộ trạng thái sổ cái một cách nhanh chóng mà không làm gián đoạn toàn bộ hệ thống.

    \item \textbf{Tính tối ưu tài nguyên:} 
    Khác với các nền tảng blockchain công nghiệp đòi hỏi cấu hình phần cứng lớn, Khoai Chain phải tối ưu hóa mức tiêu thụ tài nguyên (CPU, RAM và dung lượng ổ cứng). Một nút hệ thống phải có khả năng khởi chạy ổn định ngay cả trên các thiết bị máy tính cá nhân hoặc máy chủ ảo cấu hình thấp, phục vụ hiệu quả cho môi trường thử nghiệm và giáo dục.
...
\begin{itemize}
    \item \textbf{Khả năng tự động khám phá nút:} Mạng lưới hiện tại đang dựa vào cơ chế định tuyến tĩnh thông qua IP được cấu hình sẵn trong Docker. Trong tương lai, hệ thống cần tích hợp giao thức DHT (Distributed Hash Table) để các nút có thể tự động tìm thấy nhau trên môi trường Internet mở.
    \item \textbf{Cơ chế hoàn tác và tái sắp xếp chuỗi phức tạp:} Hệ thống hiện tại đã ngăn chặn được các nhánh rác khi đồng bộ, nhưng nếu mạng bị đứt gãy vật lý và tạo ra hai chuỗi song song dài bằng nhau, Khoai Chain chưa có cơ chế hồi phục toàn diện để tự động gỡ bỏ các khối bị sai lệch và đưa toàn mạng về một trạng thái đồng thuận duy nhất.
...
Đồ án đã hoàn thành mục tiêu nghiên cứu và xây dựng thành công Khoai Chain – một nền tảng blockchain riêng tư tinh gọn, giải quyết triệt để sự cồng kềnh của các hệ thống công nghiệp như Hyperledger Fabric. Tính đến giai đoạn hiện tại, hệ thống đã hoàn thiện và chốt hạ các chức năng cốt lõi sau:
...
\begin{itemize}
    ...
    \item \textbf{Quản lý vùng chờ giao dịch và hợp đồng thông minh:} Hoàn thiện luồng thực thi giao dịch an toàn với cơ chế tách biệt vùng nhớ đệm (\texttt{stagingState}). Hệ thống chỉ chốt trạng thái sổ cái khi toàn bộ lô khối hợp lệ, loại bỏ rủi ro dữ liệu rác.
    \item \textbf{Triển khai container hóa và SDK:} Đóng gói thành công tiến trình nút vào môi trường Docker và xây dựng bộ công cụ giao diện dòng lệnh Khoai Chain, cho phép khởi chạy, quản trị và tương tác với mạng lưới chỉ qua các thao tác lệnh đơn giản.
\end{itemize}

\subsection*{Định hướng hoàn thiện mã nguồn}
\addcontentsline{toc}{subsection}{Định hướng hoàn thiện mã nguồn}
Dựa trên nền tảng kiến trúc đã thiết lập, để Khoai Chain thực sự sẵn sàng cho môi trường triển khai thực tế của doanh nghiệp, mã nguồn cần tiếp tục được nghiên cứu và nâng cấp ở các khía cạnh sau:
